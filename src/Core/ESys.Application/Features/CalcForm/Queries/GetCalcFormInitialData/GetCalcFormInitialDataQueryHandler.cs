...
public class GetCalcFormInitialDataQueryHandler : IRequestHandler<GetCalcFormInitialDataQuery, CalcFormInitialDataVm>
{
    ...
    public async Task<CalcFormInitialDataVm> Handle(GetCalcFormInitialDataQuery request, CancellationToken cancellationToken)
    {
        var requestJson = Newtonsoft.Json.JsonConvert.SerializeObject(request.BizList);
        return DynaCalcByExp(requestJson);
    }

    public CalcFormInitialDataVm DynaCalcByExp(string requestJson)
    {
        ...
        var biz = _bizRepository.LoadBizFromDB(dataPool["BizId"]);
        if (biz != null)
            _biz = biz;
        else
        ...
        prepair(_biz.Exp, EXP);
        prepair(_biz.Func, FUNC);
        prepair(_biz.Lookup, LOOKUP);
    ...
    private void RequestDBforXML(Dictionary<string, string> lookupStr)
    {
        ...
        {
            string queryStatement = "SELECT xmlTitles, xmlTags, xmlColCount, whereClause FROM " + xmlTable + " WHERE BizId = '" + _biz.BizId + "' and tname = '" + lookupStr["table"] + "'";//queryBuilder(lookupStr);
            string xmlTitles = "";
            string xmlTags = "";
    ...
    private bool loadBizDataFromDB_(string data)
    {
        ...
        if (true /*data exists in DB*/ )
        {
            ...
            {
                _biz.Exp = "[{'shipping':{'exp':'((area*qty*thick*0.00019*250))*9200'}},{'weight':{'exp':'(area*qty*thick*0.00019)'}},{'date':{'exp':'10'}}]";
                return true;
            }
